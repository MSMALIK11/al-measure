...
export default function MapApp({
  onFeatureDrawn,
  initialFeature, // 👈 pass your GeoJSON feature here
}: {
  onFeatureDrawn?: (f: any) => void
  initialFeature?: any
}) {
  const mapRef = useRef<Map | null>(null)
  const sourceRef = useRef<VectorSource | null>(null)
  ...
  useEffect(() => {
    ...
    // ✅ If initial feature provided, load it
    if (initialFeature) {
      const format = new GeoJSON()
      const feature = format.readFeature(initialFeature, {
        featureProjection: "EPSG:3857", // map projection
        dataProjection: "EPSG:3857", // your feature looks like it's already in web mercator
      })
